...
// Three-row half-block wordmark: S E C B i N (22 columns).
export const LOGO = [
  '█▀▀ █▀▀ █▀▀ █▀▄ █ █▄ █',
  '▀▀█ █▀  █   █▀▄ █ █ ▀█',
  '▀▀▀ ▀▀▀ ▀▀▀ ▀▀  ▀ ▀  ▀',
];
// Column of the "i" stroke the ember dots.
const I_COLUMN = 16;
...
/**
 * The ember row above the wordmark: a wax-red spark dotting the "i" (column
 * I_COLUMN of the logo), padded to the logo's width so centering keeps it aligned.
...
export function logoEmber(theme, glow = true) {
  ...
  return ' '.repeat(I_COLUMN) + spark + ' '.repeat(LOGO[0].length - I_COLUMN - 1);
}
